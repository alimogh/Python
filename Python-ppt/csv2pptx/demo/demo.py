...
#Modification to meet enviroment '/ffmpeg/bin/ffmpeg.exe'
...
# ----------------------------- Setup Vars --------------------------------- Modification to meet enviroment instead of / \\
slide_template = 'inputs\\Rutgers Template GSAPP.pptx'

csv_filename = 'inputs\\admin_recordings_extract1.csv'
...
SUBTEXT_FONT_SIZE_MINIMUM = 10
SUBTEXT_LINE_SPACING = 1
SUBTEXT_SPACE_BEFORE = 1
SUBTEXT_SPACE_AFTER = 0
MOVE_SUBTEXT_UP = True

STUDENT_ID = 0
FULLNAME_COL = 1 # for That I used it to calculate the full name cause the first coulm does not contain any in the calculate_name_lengths.py I added 1,2,3 to name_column to get the first middle last name

HAS_ACCOMPLISHMENTS = True
ACCOMP_TO_TRACK = 2 # Useful for splitting, title slides, etc. Accomp number, not column number.
#----------------------Modification Here--------------------------------
ACCOMPLISH_1_COL = 8 # Degree
ACCOMPLISH_2_COL = 9 # Latin Hounrs
#ACCOMPLISH_3_COL = 17
#ACCOMPLISH_4_COL = 18 
AUDIO_COL = 2
IMAGE_COL = 3
...
#Modification to meet enviroment instead of  / \\
...
def get_length(input_audio):
    print(os.getcwd())
    result = subprocess.run(['ffprobe.exe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', input_audio], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
df = pd.read_csv('inputs\\admin_recordings_extract.csv')
s = df['First name']+' '+df['Middle name'].fillna('')+' '+df['Last name']
df.insert(2,'full name',s)
df.drop(columns=['First name','Middle name','Last name','Event slug','Preceding Text','Additional text','Student Recording Url'],inplace=True)
df.to_csv('inputs\\admin_recordings_extract1.csv',index=False)
            ...
            try:
                ...
                for shape in slide.placeholders:
                        ...
                        if 'Picture' in shape.name:
                            picture = slide.placeholders[shape.placeholder_format.idx]
                        else:

                            quote = slide.placeholders[shape.placeholder_format.idx]
                ...
                if QUOTE_COL != '':
                    if row[QUOTE_COL] != "":
                        ...
                        # accourding to stack overflow soulation about changing the font sizes https://stackoverflow.com/questions/54464130/how-to-change-the-font-size-of-an-python-pptx-object
                        for para in quote.text_frame.paragraphs:
