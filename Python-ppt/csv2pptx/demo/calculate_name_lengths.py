...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            print('Column Number inside loop:',name_column)
            name_column = int(a)
    ...
    with open(csv_filename, 'r', encoding='UTF-8', newline='') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                #print('Name is: ',row[2]+' '+row[3]+' '+row[4],'\n Column Number is:',name_column)row[name_column]
    ...
    if cmd == "c":
        if not os.path.exists('inputs\save1.png'):
        ...
        while True:
            save = pyautogui.locateOnScreen('inputs\save1.png')
            if save is not None:
                break
        #program.wait()
